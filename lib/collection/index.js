...
var Collection = function(db, collectionName, options) {
	...
	SourceCollection.call(
		...
		db,
		db.s.topology,
		db.s.databaseName || db.s.namespace.db,
		collectionName,
		null,
		options
	);

	this.customCountImplementation = options.customCountImplementation;
...
Collection.prototype._getUnsupportedErrorMessage = function(methodName) {
	return 'Method "' + methodName + '" for collection "' +
		(this.s.name || this.s.namespace.collection) + '" is not supported';
};
...
var deprecatedMethods = [{
	...
	name: 'findAndModify',
	alternatives: ['findOneAndUpdate', 'findOneAndReplace', 'findOneAndDelete']
}, {
	name: 'geoHaystackSearch',
	alternatives: ['geospatial queries']
}, {
	name: 'group',
	alternatives: ['aggregation framework']
}, {
	name: 'reIndex',
	alternatives: ['db.command']
}];

deprecatedMethods.forEach(function(method) {
	Collection.prototype[method.name] = generateDeprecatedMethodFunction(
		method.name,
		method.alternatives
	);
});
