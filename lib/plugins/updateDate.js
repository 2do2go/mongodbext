...
module.exports = function(collection, options) {
	...
	var beforeInsert = function(params, callback) {
		...
		var updateDate = dateFormatter.format(new Date());
		objs.forEach(function(obj) {
			if (!dateFormatter.isDate(obj.updateDate)) {
	...
	var beforeUpdate = function(params, callback) {
		...
		if (obj && !dateFormatter.isDate(obj.updateDate)) {
			obj.updateDate = dateFormatter.format(new Date());
		}
